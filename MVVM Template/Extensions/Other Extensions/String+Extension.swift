...
extension String {
    ...
    public func toBool() -> Bool? {
        let trimmedString = self.replacingOccurrences(of: " ", with: "").lowercased()
...
//        if DataManager.sharedInstance.getLanguageDirection() == "RTL" {// check RTL or LTR here
...
extension UIImage {
    ...
    func setLocalized() -> UIImage{
        if /*DataManager.sharedInstance.getLanguageDirection().lowercased()*/ "" == "ltr"{
            return self
        } else {
            return self.flipped
        }
...
extension UIImageView {
    func setLocalized(){
        self.image = self.image?.setLocalized() ?? self.image
...
//            self.setImage(currentImage.setLocalized())
